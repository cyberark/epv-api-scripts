...
$URL_Application = $URL_PVWAAPIv1+"/Applications"
$URL_ApplicationAuth = $URL_Application+"/{0}/Authentications"
$URL_ApplicationSearch = $URL_Application+"?IncludeSublocations={0}"
$URL_ApplicationSearchAppID = $URL_ApplicationSearch + "&AppID={1}"
...
Function Get-Application{
    ...
    IF ([string]::IsNullOrEmpty($AppID)){
        $url =  $URL_ApplicationSearch -f  $($IncludeSublocations)
    } else {
        $url = $URL_ApplicationSearchAppID -f  $($IncludeSublocations), $($AppID)
    }
...
Function New-Application{
    ...
    $application = @{ AppID=$AppID;Description=$Description}
...
Function New-ApplicationAuth{
    ...
    Switch($AuthType.ToLower()) {
        ...
        "certificateattr"{
            IF (![string]::IsNullOrEmpty($Issuer)) {$authentication += @{Issuer ="[$Issuer]"}}
            IF (![string]::IsNullOrEmpty($Subject)) {$authentication += @{Subject ="[$Subject]"}}
            IF (![string]::IsNullOrEmpty($SubjectAlternativeName)) {$authentication += @{SubjectAlternativeName ="[$SubjectAlternativeName]"}}
        }


    }
    $body = @{ authentication =$authentication} | ConvertTo-Json -Depth 3 -Compress
...
Function Get-ApplicationAuth{
    ...
    return Invoke-Rest -Uri $($URL_ApplicationAuth -f $AppID)-Header $g_LogonHeader -Command "Get"
