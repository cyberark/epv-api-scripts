...
If (Test-CommandExists Invoke-RestMethod) {
	...
	switch($PsCmdlet.ParameterSetName) {
		"Get" {
			...
			if($GetRuleResponse.AutomaticOnboardingRules.Length -eq 0) {
				...
				foreach($rule in $GetRuleResponse.AutomaticOnboardingRules){
					$arrRules += $($rule | Select-Object RuleId, RuleName, DecisionPlatformId, DecisionSafeName, UserNameFilter)
				}
		...
		"Create" {
			...
			$CreateRuleResponse | Select-Object RuleId, RuleName, DecisionPlatformId, DecisionSafeName, UserNameFilter | Format-Table -AutoSize
		}
