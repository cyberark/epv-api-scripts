...
$URL_Password = $URL_AccountsDetails + "/Password/Retrieve"
	...
	If ((Test-CommandExists Invoke-RestMethod) -eq $false)
 {
	...
	catch [System.Net.WebException]
	{
		if ($ErrAction -match ("\bContinue\b|\bInquire\b|\bStop\b|\bSuspend\b"))
		{
			...
			Add-Content -Path "$CsvPath.invokeError" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
			Add-Content -Path "$CsvPath.bad" -Value "Error Message: $account.name `t$_ $body"
		}
		$restResponse = $null
	}
	catch
	...
	if ($null -ne $account)
	{
		...
		try
		{
			...
			elseif (($NoSafeCreation -eq $True) -and ($safeExists -eq $false))
			{
				...
				Add-Content -Path "$CsvPath.noSafe" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
			...
			If ($shouldSkip -eq $False)
			{
				...
				try
				{
					If ($accExists)
					{
						...
						If ($Update)
						{
							...
							{
								Add-Content -Path "$CsvPath.propSkip" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
							...
							else
							{
								...
								{
									Add-Content -Path "$CsvPath.propUpdate" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
							...
							If (![string]::IsNullOrEmpty($objAccount.secret))
							{
								...
								if ($objAccount.secretType -eq "password")
								{
									...
									$urlaccountPass = $URL_Password -f $s_Account.id
									$body = "{`"reason`":`"Checking Password Sync with On-Prem`"`n`}"

									$accountPassCheck = $(Invoke-Rest -Uri $urlaccountPass -Header $g_LogonHeader -Body $body -Command "Post")

									if ($accountPassCheck.trim() -ne $objAccount.secret.trim())
									{
										$accountPassCheck = $null
										...
										if ($null -ne $UpdateAccountResult)
										{
											Add-Content -Path "$CsvPath.secretUpdate" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
									...
									{
										Write-LogMessage -Type Warning -MSG "New Account Secret matchs current value - skipping" 
										Add-Content -Path "$CsvPath.secretmatch" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
										$accountPassCheck = $null
									}
						...
						ElseIf ($Create)
						{
							try
							{
								...
								Write-Warning "The Account Exists, Creating the same account twice will cause duplications" -WarningAction Stop
						...
						ElseIf ($Delete)
						{
							...
							if ($null -ne $DeleteAccountResult)
							{
								# Increment counter
								$counter++
								Add-Content -Path "$CsvPath.delete" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
					...
					if ($createAccount)
					{
						try
						{
							...
							if ($null -ne $addAccountResult)
							{
								...
								Add-Content -Path "$CsvPath.create" -Value ($account | ConvertTo-Csv -NoTypeInformation) 
								# Increment counter
								$counter++
				...
				catch
				{
					...
					Add-Content -Path "$CsvPath.error" -Value ($account | ConvertTo-Csv -NoTypeInformation |Select-Object -Skip 1 ) 
					Add-Content -Path "$CsvPath.bad" -Value "There was an error onboarding $g_LogAccountName (CSV line: $csvLine) into the Password Vault. Error: $(Join-ExceptionMessage $_.Exception)" 
				}
			}
			else
			{
				...
				Add-Content -Path "$CsvPath.noSafe" -Value ($account | ConvertTo-Csv -NoTypeInformation) 
			}
		}
		catch
		{
