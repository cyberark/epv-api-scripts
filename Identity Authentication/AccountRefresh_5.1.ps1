...
$usersToRefresh = Get-ADGroupMember "$GroupName" |Get-ADUser |Select-Object UserPrincipalName,ObjectGUID
} Elseif (!$([string]::IsNullOrEmpty($UPN))) {
    $usersToRefresh =  get-aduser -filter  "UserPrincipalName -like `"$UPN`"" |Select-Object -Property UserPrincipalName,ObjectGUID
} else {
	Write-LogMessage -type Error "No group or user passed. Re-run and use -UPN to define a single user by UserPrincipalName or -GroupName to refresh all users in a Active Directory group"
	continue
}
