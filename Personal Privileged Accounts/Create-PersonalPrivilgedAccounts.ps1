...
#
# COMMENT: 
# This script will create personal privielged accounts according to a CSV
...
#>
    ...
    try {
        If([string]::IsNullOrEmpty($RadiusOTP))
        {
            ...
            # Use the same header using the RADIUS OTP (so we dont have to request the OTP every login)
...
#>
	...
	{
        Throw $(New-Object System.Exception ("Test-Safe: Error testing safe '$safeName' existance.",$_.Exception))
	}
}
...
# Start Bulk onboarding all collected personal privilged accounts
try{
    If($personalPrivAccounts.Count -gt 0)
    {

        Write-LogMessage -Type Info -Msg "Starting onboarding $($personalPrivAccounts.Count) personal privielged accounts"
        $authHeader = Get-LogonHeader -Credentials $creds -RadiusOTP $RadiusOTP
        $bulkBody = @{
        ...
        Write-LogMessage -Type Info -Msg "No personal privielged accounts to onboard"
    }
} catch {
